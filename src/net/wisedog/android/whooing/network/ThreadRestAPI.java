...
public class ThreadRestAPI extends Thread {
	...
	private void checkLoginInfo() {	    
        SharedPreferences prefs = WhooingApplication.getInstance().
                getApplicationContext().getSharedPreferences(Define.SHARED_PREFERENCE, 0);    //0 is MODE_PRIVATE
        ...
        Define.LOCALE_LANGUAGE = prefs.getString(Define.KEY_SHARED_LOCALE_LANGUAGE, "");
        Define.TIMEZONE = prefs.getString(Define.KEY_SHARED_TIMEZONE, null);
		if (Define.DEBUG == true) {
			Log.i("wisedog", "user_id: " + Define.USER_ID + " app_section : "
					+ Define.APP_SECTION + " real_token:" + Define.REAL_TOKEN
					+ " pin : " + Define.PIN + " token_secret : "
					+ Define.TOKEN_SECRET);
			Log.i("wisedog", "country: " + Define.COUNTRY_CODE + " currency: "
					+ Define.CURRENCY_CODE + " Locale : "
					+ Define.LOCALE_LANGUAGE + " Timezone: " + Define.TIMEZONE);
		}
	...
	public void run() {
		...
		int boardType = BbsListFragment.BOARD_TYPE_FREE;
		String type = "";
		if(mAPIName == Define.API_GET_BOARD 
				|| mAPIName == Define.API_GET_BOARD_ARTICLE
				|| mAPIName == Define.API_POST_BOARD_COMMENT 
				|| mAPIName == Define.API_GET_BOARD_COMMENT){
		    if (mBundle == null) {
	        ...
	        if(boardType == BbsListFragment.BOARD_TYPE_FREE){
	            type = "free";
	        }else if(boardType == BbsListFragment.BOARD_TYPE_MONEY_TALK){
	            type = "moneytalk";
	        }else if(boardType == BbsListFragment.BOARD_TYPE_COUNSELING){
	            type = "counseling";
	        }else if(boardType == BbsListFragment.BOARD_TYPE_WHOOING){
	            type = "whooing";
	        }
		}
		if(Define.DEBUG){
		...
		case Define.API_GET_BOARD_COMMENT:
		{
            ...
            requestUrl = "https://whooing.com/api/bbs/" + type + "/" + bbsId + "/" + commentId + ".json";
